...
export async function fetchManifest(slug) {
  ...
  try {
    ...
    throw new ManifestError('Manifest file is not valid JSON.', {
      cause: error,
      url
    });
  }
...
export function validateManifest(data, { slug = null, url = null } = {}) {
  if (!data || typeof data !== 'object' || Array.isArray(data)) {
    throw new ManifestError('Manifest must be a JSON object.', { url });
  }
  ...
  if (errors.length) {
    throw new ManifestError(errors.join(' '), { url, details: { errors } });
  }
...
function buildManifestUrl(slug) {
  ...
  return resolveAppUrl(`ar/${sanitized}/manifest.json`);
}
