...
function handleManifestError(error) {
  ...
  if (state.slug) {
    const expectedUrl =
      (error instanceof ManifestError && error.url) ||
      resolveAppUrl(`ar/${encodeURIComponent(state.slug)}/manifest.json`);
    let expectedPath = expectedUrl;
    try {
      expectedPath = new URL(expectedUrl).pathname;
    } catch (urlError) {
      // Ignore URL parsing issues and fall back to the raw value.
    }
    detail = `${detail} (looked for ${expectedPath})`;
  }
...
function resolveManifestAsset(path) {
  ...
  try {
    const baseUrl = state.manifest.url || resolveAppUrl('');
...
async function ensureARLibsLoaded() {
  ...
  if (!arLibsPromise) {
    arLibsPromise = (async () => {
      await loadScript(resolveAppUrl('libs/aframe.min.js'));
      await loadScript(resolveAppUrl('libs/mindar-image-aframe.prod.js'));
    })();
  }
  await arLibsPromise;
}
